...
const History = () => {
  ...
  useEffect(() => {
    const getHistory = async () => {
      try {
        const res = await axios.get('https://farm2table-production.up.railway.app/api/user_orders', {
  ...
  useEffect(() => {
    axios.get('https://farm2table-production.up.railway.app/api/farmer/orders', {
      headers: { Authorization: token }
  ...
  const updateStatus = async (id, status) => {
    await axios.put(`https://farm2table-production.up.railway.app/api/farmer/orders/${id}/status`, { status }, {
      headers: { Authorization: token }
    });
